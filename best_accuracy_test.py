...
def validate_symbol(symbol, initial_train_days=300, test_days=14, max_tests=15):
    ...
    for i in range(max_tests):
        ...
        try:
            sr = RobustScaler(); sc = RobustScaler()
            ...
            reg_lgb = LGBMRegressor(
                ...
                reg_alpha=0.05, reg_lambda=1.5, random_state=42, n_jobs=-1, verbose=-1)
        ...
        clf_probs = (clf_xgb.predict_proba(X_te_c)[:, 1] +
                     clf_lgb.predict_proba(X_te_c)[:, 1]) / 2
    ...
    conf_mask  = (probs_arr >= 0.60) | (probs_arr <= 0.40)
    ...
    return {
        'symbol':       symbol,
        'n_preds':      len(all_actual),
        'mape':         mape,
        'price_acc':    price_acc,
        'dir_acc_all':  dir_acc_all,
        'dir_acc_conf': dir_acc_conf,
        'conf_pct':     conf_pct,
    }
