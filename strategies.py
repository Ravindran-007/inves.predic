...
def bollinger_bands_strategy(df, window=20, num_std=2):
    ...
    df['Signal'] = 0
    df.loc[close < df['Bollinger_Lower'], 'Signal'] = 1
    df.loc[close > df['Bollinger_Upper'], 'Signal'] = -1
    return df
...
def backtest(prices, signals, initial_capital=10000):
    ...
    return (equity, net_profit, {'Initial Capital': initial_capital, 'Final Equity': round(float(equity.iloc[-1]), 2), 'Net Profit': round(float(net_profit), 2), 'Return (%)': round(net_profit / initial_capital * 100, 2), 'Max Equity': round(float(equity.max()), 2), 'Min Equity': round(float(equity.min()), 2)})